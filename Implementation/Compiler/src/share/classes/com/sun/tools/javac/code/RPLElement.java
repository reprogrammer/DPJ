...
public abstract class RPLElement {
    ...
    /**
     * An RPL that bounds this element from above, used in the disjointness
     * test.
     */
	...
	/**
	 * RPL in which this parameter is included -- used for capture
	 * parameters
	 */
	...
	public boolean equals(Object o) {
	    if (!(o instanceof ArrayIndexRPLElement))
		return false;
	...
	public boolean isDisjointFrom(RPLElement e, RPLs rpls,
		...
		return true;
	    return areNeverEqualExprs(indexExp,
		    ((ArrayIndexRPLElement) e).indexExp);
	}
	...
	public static boolean areAlwaysEqualExprs(JCExpression first,
		JCExpression second) {
		...
		return (areAlwaysEqualExprs(firstBinary.lhs, secondBinary.lhs) && areAlwaysEqualExprs(
			firstBinary.rhs, secondBinary.rhs));
	    }
	    if (first instanceof JCLiteral && second instanceof JCLiteral) {
		return ((JCLiteral) first).getValue().equals(
			((JCLiteral) second).getValue());
	    }
		...
		if (firstSymbol != secondSymbol)
		    return false;
		if ((firstSymbol.flags() & Flags.FINAL) != 0)
		    return true;
		return false;
	    }
	    if (first instanceof JCFieldAccess
		    && second instanceof JCFieldAccess) {
		JCFieldAccess fa1 = (JCFieldAccess) first;
		JCFieldAccess fa2 = (JCFieldAccess) second;
		return areAlwaysEqualExprs(fa1.selected, fa2.selected)
			&& fa1.sym == fa2.sym;
	    }
	    return first == second;
	}

	public static boolean areNeverEqualExprs(JCExpression first,
		JCExpression second) {
	    if (first instanceof JCLiteral && second instanceof JCLiteral) {
		return !((JCLiteral) first).getValue().equals(
			((JCLiteral) second).getValue());
     ...
     * specified RPLs turn into these parameters. See the tech report for more
     * details.
     */
    ...
    /**
     * A class for undetermined region parameters, used during parameter
     * inference in resolving call sites.
     */
	...
	public boolean equals(Object o) {
	    if (!(o instanceof StackRPLElement))
		return false;
