 ...
 * @see Resolve
 * @see ConstFold
 * @see Infer ListBuffer<T> lb = new ListBuffer<T>(); for (T tree: trees)
 *      lb.append(copy(tree, p)); return lb.toList();
 * 
 * 
 *      <p>
 *      <b>This is NOT part of any API supported by Sun Microsystems. If you
 *      write code that depends on this, you do so at your own risk. This code
 *      and its internal interfaces are subject to change or deletion without
 *      notice.</b>
 */
public class Attr extends JCTree.Visitor {
    ...
    /**
     * A small pass that happens after Enter and before Attr. It scans the tree
     ...
     * 
     * 3. Check satisfaction of RPL and effect constraints for types appearing
     * in class members.
     * 
     * Because RPLs use global symbols, these operations must wait until after
     * Enter is finished, or the symbols they need may not be in scope. Yet they
     * cannot happen during attribution, because their results are needed by
     * attribution and/or effect checking (which is interleaved with attribution
     * on a class-by-class basis). So we insert a pass between Enter and Attr to
	...
	public void visitMethodDef(JCMethodDecl tree) {
		...
		// Enter type, region, and effect parameters into the local
		// scope.
	...
	public void visitTypeApply(JCTypeApply tree) {
		...
		if (cs.constraints != null) {
		    // Check that disjointness constraints on region args are
		    // satisfied
		    if (!rpls.disjointnessConstraintsAreSatisfied(
			    cs.constraints.disjointRPLs, ct.getRegionParams(),
			    ct.getRegionActuals(),
			    parentEnv.info.constraints.disjointRPLs)) {
			enter.log.warning(tree, "rpl.constraints");
		    }
		    // Check that nonint constraints on effect vars are
		    // satisfied
		    if (!Effects.nonintConstraintsAreSatisfied(
			    cs.constraints.noninterferingEffects, ct,
			    parentEnv.info.constraints)) {
     ...
     */
    protected Type check(JCTree tree, Type owntype, int ownkind, int pkind,
	    Type pt) {
	if (ownkind == VAR && pkind == RPL_ELT) {
	    // OK, using a variable as a z region RPL element
	} else if (owntype.tag != ERROR && pt.tag != METHOD && pt.tag != FORALL) {
	    if ((ownkind & ~pkind) == 0) {
	...
	localEnv.enclClass = make.ClassDef(make.Modifiers(0),
		syms.errSymbol.name, null, null, null, null, null);
    ...
    /**
     * Convert an expression to an RPL. Used in substituting for 'this' in
     * expressions like e.f, where f is declared in region this.
     */
	...
	if (sym instanceof VarSymbol
	    ...
	    return new RPL(List.<RPLElement> of(new RPLElement.VarRPLElement(
		    (VarSymbol) sym)));
	}
	// Otherwise, use the owner region
	RPL owner = ((ClassType) tree.type).getOwner();

	// DPJIZER: Replaced return new RPL(owner.elts.append(RPLElement.STAR));
	// by the following.
	return owner.appenedByStar();
    }
    ...
    // FIXME: Added just to let the porting tool access method entry
    // environments.
     ...
     */
    protected Type attribTree(JCTree tree, Env<AttrContext> env, int pkind,
	    Type pt) {
     ...
     */
    protected List<Type> attribExprs(List<JCExpression> trees,
	    Env<AttrContext> env, Type pt) {
	...
	if (tree.paramInfo != null) {
	    tree.sym.constraints = enterRegionParamInfo(tree.paramInfo,
		    localEnv);
	}
     ...
     */
    public Constraints enterRegionParamInfo(DPJParamInfo tree,
	...
	for (Pair<DPJRegionPathList, DPJRegionPathList> treeConstraint : tree.rplConstraints) {
	    Pair<RPL, RPL> constraint = new Pair<RPL, RPL>(
		    treeConstraint.fst.rpl, treeConstraint.snd.rpl);
	    rplBuf.append(constraint);
	}
	List<Pair<RPL, RPL>> rplConstraints = rplBuf.toList();
	localEnv.info.constraints.disjointRPLs = localEnv.info.constraints.disjointRPLs
		.appendList(rplConstraints);
	...
	for (Pair<DPJEffect, DPJEffect> treeConstraint : tree.effectConstraints) {
	    Pair<Effects, Effects> constraint = new Pair<Effects, Effects>(
		    treeConstraint.fst.effects, treeConstraint.snd.effects);
	    effectBuf.append(constraint);
	}
	List<Pair<Effects, Effects>> effectConstraints = effectBuf.toList();
	localEnv.info.constraints.noninterferingEffects = localEnv.info.constraints.noninterferingEffects
		.appendList(effectConstraints);
	...
	try {
	    ...
	    methodEntryEnv = localEnv.dup(tree,
		    localEnv.info.dup(localEnv.info.scope.dupUnshared()));
	    ...
	    /*
	     * if (tree.rgnParamInfo != null) { attribTree(tree.rgnParamInfo,
	     * localEnv, NIL, Type.noType); }
	     */
	...
	try {
		...
		if ((v.flags_field & FINAL) != 0
		    ...
		    // In this case, `v' is final. Ensure that its initializer
		    // is
		    // evaluated.
	...
	if (isConstructorCall) {
		...
		if (site.tag == CLASS) {
		    ...
		    Symbol sym = rs.resolveConstructor(tree.meth.pos(),
			    localEnv, site, argtypes, typeargtypes, regionargs,
			    effectargs);
	    ...
	    Type mpt = newMethTemplate(argtypes, typeargtypes, regionargs,
		    effectargs);
	    localEnv.info.varArgs = false;
		...
		if (!rpls.disjointnessConstraintsAreSatisfied(
			methSym.constraints.disjointRPLs, methSym.rgnParams,
		...
		if (!Effects.nonintConstraintsAreSatisfied(
			methSym.constraints.noninterferingEffects, tree,
			methSym.rgnParams, regionargs, methSym.effectparams,
			effectargs, types, this, env)) {
		    enter.log.warning(tree, "effect.constraints");
		}
	    }
	...
	return ((typeargtypes == null) && (regionargs == null) && (effectargs == null)) ? mt
		: (Type) new ForAll(typeargtypes, regionargs, effectargs, mt);
    }
	...
	(clazz.getTag() == JCTree.TYPEAPPLY) ? ((JCTypeApply) clazz).functor
		: clazz;
	...
	if (tree.encl != null) {
		...
		clazz = make.at(tree.pos).RegionApply(clazzid1,
	...
	if (clazztype.tag == CLASS) {
		...
		Symbol sym = rs.resolveConstructor(tree.pos(), localEnv,
			clazztype, argtypes, typeargtypes, regionargs,
			effectargs, true, tree.varargsElement != null);
	...
	if (tree.elemtype != null) {
		...
		if (indexVars.head != null) {
		    indexVar = new VarSymbol(0, indexVars.head.name,
		...
		owntype = new ArrayType(owntype, rplSyms.head, indexSyms.head,
			syms.arrayClass);
	...
	if (!types.isSubtype(exprtype, clazztype)
	    ...
	    // HACK ALERT: We want to warn about unsound casts, but we need to
	    // suppress
	    // all the warnings caused by the DPJ runtime, or those warnings
	    // swamp the
	    // real ones and make the real ones useless. Once we have separate
	    // compilation,
	    // this won't be an issue, because we won't need to recompile the
	    // DPJ runtime
	    // whenever we compile the user program.
	    String tstring = env.enclClass.sym.type.toString();
	    if (tstring.length() < 10
		    || !tstring.substring(0, 10).equals("DPJRuntime")) {
		// Turn this off for now because it causes warnings in the
		// bootstrap
	...
	if (types.isArray(atype)) {
	    ...
	    if (at.indexVar != null) {
		owntype = types.substIndices(owntype,
			List.<VarSymbol> of(at.indexVar),
	...
	Env<AttrContext> env1 = env;
	if (sym.kind != ERR && sym.owner != null
	...
	env.info.siteVar = (selectedSym instanceof VarSymbol) ? (VarSymbol) selectedSym
		: null;
     ...
     */
    protected Symbol selectSym(JCFieldAccess tree, // DPJ: Changed private to
						   // protected
	    Type site, Env<AttrContext> env, Type pt, int pkind) {
     ...
     *            The expected type.
     */
    protected Type checkId(JCTree tree, Type site, Symbol sym,
	    Env<AttrContext> env, int pkind, Type pt, boolean useVarargs) {
	if (pt.isErroneous())
	    return syms.errType;
	...
	case TYP:
		...
		if (owntype.tsym.type.getTypeArguments().nonEmpty()
		    ...
		    List<RegionParameterSymbol> regionParams = owntype
			    .getRegionParams();
		    ...
		    // position.
		    // If this type is subject to a region apply, the ROOT's
		    // will
		    // get replaced. Otherwise, we have a type instantiated
		    // with ROOT's. This is the DPJ equivalent of a Java
		    // "raw type."
		    ListBuffer<RPL> buf = ListBuffer.lb();
    ...
    public Type checkMethod(Type site, Symbol sym, Env<AttrContext> env,
	    ...
	    List<Effects> effectargs, boolean useVarargs) {
	...
	if (owntype == null) {
		...
		log.error(
			...
			sym,
			site,
			RPL.toString(pt.getRegionActuals())
	...
	if (tree.indexParam != null) {
	    indexVar = new VarSymbol(STATIC, tree.indexParam.name,
		    syms.intType, scope.owner);
	...
	if (functortype.tag == TYPEVAR) {
	    ...
	    // types
	    List<DPJRegionPathList> rplArgs = (tree.rplArgs == null) ? List
		    .<DPJRegionPathList> nil() : tree.rplArgs;
		...
		owntype = newtvar = new TypeVar(tvartype.tsym,
			tvartype.getUpperBound(), tvartype.lower);
	...
	if (functortype.tag == CLASS) {
	    ...
	    List<Type> typeFormals = functortype.tsym.type.getTypeArguments();

	    // Get the formal region params
	    List<RegionParameterSymbol> rplFormals = functortype.tsym.type
		    .getRegionParams();

	    // Get the formal effect params
	    List<Effects> effectFormals = functortype.tsym.type
		    .getEffectArguments();

	    // Handle error case of unexpected args
	    if (typeFormals.isEmpty() && rplFormals.isEmpty()
		    && effectFormals.isEmpty() && tree.typeArgs.nonEmpty()) {
	    ...
	    // Use the # of type params to separate the type args from the RPL
	    // args
	    int counter = 0;
		...
		// We already have rplargs in the tree, so there shouldn't be
		// any
	    ...
	    if (tree.effectArgs == null)
		tree.effectArgs = List.nil();
	    ...
	    // Construct the instantiated type with the type, RPL, and effect
	    // args
	    owntype = new ClassType(clazzOuter, actuals, rplFormals,
		    rplActuals, effectActuals, functortype.tsym);
	}
	result = check(tree, owntype, TYP, pkind, pt);
    ...
    /**
     * Helper function: Make a list of expressions, some of which may have been
     * parsed as types, into a list of RPLs.
     */
	...
	for (JCExpression expr : trees) {
		...
		// If we parsed the argument as a type, then make it into an RPL
		// now.
		DPJRegionPathListElt elt = make.at(expr.pos).RegionPathListElt(
			expr, DPJRegionPathListElt.NAME);
	...
	if (bs.length() > 1) {
	    ...
	    JCClassDecl cd = make.at(tree.pos).ClassDef(
		    make.Modifiers(PUBLIC | ABSTRACT), tree.name, null,
    ...
    public void visitRegionDecl(DPJRegionDecl tree) { // DPJ -- based on
						      // visitVarDef
	// Local variables have not been entered yet, so we need to do it now:
	if (env.info.scope.owner.kind == MTH) {
	    if (tree.sym != null) {
    ...
    /**
     * Visitor method for region parameter declarations, including disjointness
     * constraints.
     */
	...
	for (DPJRegionPathListElt elt : tree.elts) {
		...
		if (firstPos) {
			...
			// If RPL doesn't start with 'Root', a parameter, a
			// variable,
			// or a local name, then prepend the implicit 'Root :'
			buf.append(RPLElement.ROOT_ELEMENT);
		    }
		    /*
		     * if (elt.rplElt.isLocalName()) { // If RPL starts with a
		     * local region name, then it // implicitly starts with
		     * 'Root : Local :' buf.append(RPLElement.LOCAL_ELEMENT); }
		     */
		} else {
		    // Otherwise ...
		    if (elt.rplElt == RPLElement.ROOT_ELEMENT)
	...
	case DPJRegionPathListElt.NAME:
		...
		if (id.name == names.root) {
			...
			elt.rplElt = new RPLParameterElement(
				(RegionParameterSymbol) sym);
		    else if (sym instanceof VarSymbol) {
			VarSymbol vsym = (VarSymbol) sym;
			elt.rplElt = new RPLElement.VarRPLElement(vsym);
			if ((vsym.flags() & Flags.FINAL) == 0
				|| (vsym.name != names._this && vsym.owner.kind != Kinds.MTH))
			    log.error(elt.pos,
				    "region.must.be.final.local.variable");
		    }
		}
	    } else if (elt.exp instanceof JCFieldAccess) {
		JCFieldAccess fieldAccess = (JCFieldAccess) elt.exp;
		Type selectedType = attribTree(fieldAccess.selected, env, TYP,
			Type.noType);
		// from Attr#selectSym
		Symbol sym = rs.findIdentInType(env, selectedType,
			fieldAccess.name, RPL_ELT);
		if ((pkind & ERRONEOUS) == 0)
		    sym = rs.access(sym, fieldAccess.pos(), selectedType,
			    fieldAccess.name, true);
		fieldAccess.sym = sym;
		if (fieldAccess.sym instanceof RegionNameSymbol)
		    elt.rplElt = new NameRPLElement(
			    (RegionNameSymbol) fieldAccess.sym);
		else if (fieldAccess.sym instanceof VarSymbol)
		    elt.rplElt = new RPLElement.VarRPLElement(
			    (VarSymbol) fieldAccess.sym);
	    } else {
    ...
    /**
     * Visitor method for region parameters applied to a region-parameterized
     * type.
     */
    public void visitRegionApply(DPJRegionApply tree) {
	Type owntype = syms.errType;
	...
	if (clazztype.tag == CLASS) {
	    List<RegionParameterSymbol> formals = clazztype.tsym.type
		    .getRegionParams();
		...
		owntype = new ClassType(clazzOuter, ct.typarams_field, formals,
	...
	if (tree.isPure) {
	    // Nothing to do
	} else if (tree.readEffects.nonEmpty() || tree.writeEffects.nonEmpty()
		...
		if (treeEffectParam.sym instanceof EffectParameterSymbol) {
		    tree.effects.add(new Effect.VariableEffect(
			    (EffectParameterSymbol) treeEffectParam.sym));
		}
	    }
	} else {
