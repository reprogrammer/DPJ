...
public class Resolve {
	...
	effectNotFound = new ResolveError(ABSENT_EFFECT, syms.errSymbol,
		"effect not found");
	...
	case PRIVATE:
	    return env.enclClass.sym.outermostClass() == c.owner
		    .outermostClass();
	case 0:
	    return env.toplevel.packge == c.owner // fast special case
		    || env.toplevel.packge == c.packge()
		    ||
		    // Hack: this case is added since synthesized default
		    // constructors
	...
	case PROTECTED:
	    ...
	    env.info.selectSuper && (sym.flags() & STATIC) == 0
		    && sym.kind != TYP)
		    && isAccessible(env, site) &&
		    ...
		    (sym.kind != MTH || sym.isConstructor() || ((MethodSymbol) sym)
			    .implementation(site.tsym, types, true) == sym);
     ...
     * arguments and argument types. If successful, return the method's
     ...
     * instance method seen as a member of underdetermined site In this case, we
     ...
     * @param regionargs
     *            The invocation's given region arguments.
     * @param allowBoxing
	...
	if (mt.tag != FORALL && typeargtypes.nonEmpty()) {
		...
		List<Type> bounds = types.subst(
			types.getBounds((TypeVar) formals.head), pmt.tvars,
			typeargtypes);
	...
	if (mt_old.tag == FORALL
		&& (regionargs.nonEmpty() || effectargs.nonEmpty())) {
	    ...
	    mt = pmt.qtype;
	    mt = types.substRPL(mt, pmt.tvars, typeargtypes, formals,
		    regionargs);
	...
	if (instNeeded) {
	    ...
	    Type result = infer.instantiateMethod(tvars, (MethodType) mt,
		    argtypes, allowBoxing, useVarargs, warn);
	...
	if (env.info.actualArgs != null && params != null) {
	    paramTypes = types.substIndices(paramTypes, params,
		    env.info.actualArgs);
	}
     ...
     */
    boolean argumentsAcceptable(List<Type> argtypes, List<Type> formals,
	...
	while (argtypes.nonEmpty() && formals.head != varargsFormal) {
	    boolean works = allowBoxing ? types.isConvertible(argtypes.head,
		    formals.head, warn) : types.isSubtypeUnchecked(
		    argtypes.head, formals.head, warn);
	    if (!works)
		return false;
     ...
     * which the symbol is a member name is the symbol's name if no arguments
     * are given argtypes are the value arguments, if we search for a method
     * 
     ...
     *            or implemented interface of site's class.
     */
    Symbol findFieldRegion(Env<AttrContext> env, Type site, Name name,
	    TypeSymbol c) {
	...
	if (sym.kind == VAR)
	    ...
	    throw new FatalError(JCDiagnostic.fragment(
		    "fatal.err.cant.locate.field", name));
    }
	...
	for (; e.scope != null; e = e.next()) {
		...
		return isAccessible(env, origin, sym) ? sym : new AccessError(
			env, origin, sym);
	    }
	}
	...
	while (env1.outer != null) {
		...
		e = e.next();
	    sym = (e.scope != null) ? e.sym : findFieldRegion(env1,
		    env1.enclClass.sym.type, name, env1.enclClass.sym);
	    if (e.sym instanceof RegionParameterSymbol)
		sym = e.sym;
	    if (sym.exists()) {
		if (staticOnly && ((sym.kind & RPL_ELT) != 0)
			&& sym.owner.kind == TYP && (sym.flags() & STATIC) == 0)
		    return new StaticError(sym);
		else
		    return sym;
	    } else if (sym.kind < bestSoFar.kind) {
	...
	sym = findFieldRegion(env, syms.predefClass.type, name,
		syms.predefClass);
	if (sym.exists())
	    return sym;
	if (bestSoFar.exists())
	...
	if ((bestSoFar.kind & RPL_ELT) != 0
		&& bestSoFar.owner.type != origin.type)
	    return bestSoFar.clone(origin);
	else
	    return bestSoFar;
    }
    ...
    Symbol selectBest(Env<AttrContext> env, Type site, List<Type> argtypes,
	    ...
	    List<Effects> effectargs, Symbol sym, Symbol bestSoFar,
	...
	return (bestSoFar.kind > AMBIGUOUS) ? sym : mostSpecific(sym,
		bestSoFar, env, site, allowBoxing && operator, useVarargs);
    }

    /*
     * Return the most specific of the two methods for a call, given that both
     ...
     */
    Symbol mostSpecific(Symbol m1, Symbol m2, Env<AttrContext> env, Type site,
	...
	case MTH:
		...
		if (!types.isSameType(m1.erasure(types), m2.erasure(types)))
		    return new AmbiguityError(m1, m2);
		// both abstract, neither overridden; merge throws clause and
		// result type
		Symbol result;
		Type result2 = mt2.getReturnType();
		;
		if (mt2.tag == FORALL)
		    result2 = types.subst(result2, ((ForAll) mt2).tvars,
			    ((ForAll) mt1).tvars);
	...
	case AMBIGUOUS:
	    ...
	    Symbol err1 = mostSpecific(m1, e.sym1, env, site, allowBoxing,
		    useVarargs);
    ...
    Symbol findMethod(Env<AttrContext> env, Type site, Name name,
	    ...
	    List<Effects> effectargs, boolean allowBoxing, boolean useVarargs,
	    boolean operator) {
	return findMethod(env, site, name, argtypes, typeargtypes, regionargs,
		effectargs, site.tsym.type, true, methodNotFound, allowBoxing,
		useVarargs, operator);
    }
    ...
    private Symbol findMethod(Env<AttrContext> env, Type site, Name name,
	    List<Type> argtypes, List<Type> typeargtypes, List<RPL> regionargs,
	    List<Effects> effectargs, Type intype, boolean abstractok,
	...
	for (Type ct = intype; ct.tag == CLASS; ct = types.supertype(ct)) {
		...
		if (e.sym.kind == MTH && (e.sym.flags_field & SYNTHETIC) == 0) {
		    bestSoFar = selectBest(env, site, argtypes, typeargtypes,
			    regionargs, effectargs, e.sym, bestSoFar,
		...
		for (List<Type> l = types.interfaces(c.type); l.nonEmpty(); l = l.tail) {
		    bestSoFar = findMethod(env, site, name, argtypes,
			    typeargtypes, regionargs, effectargs, l.head,
			    abstractok, bestSoFar, allowBoxing, useVarargs,
			    operator);
		}
	...
	sym = findMethod(env, syms.predefClass.type, name, argtypes,
		typeargtypes, regionargs, effectargs, allowBoxing, useVarargs,
		false);
	if (sym.exists())
	    return sym;
	...
	for (; e.scope != null; e = e.next()) {
		...
		bestSoFar = selectBest(env, origin, argtypes, typeargtypes,
			regionargs, effectargs, sym, bestSoFar, allowBoxing,
			useVarargs, false);
	    }
	}
	if (bestSoFar.exists())
     ...
     * @param kind
     *            Indicates the possible symbol kinds (a subset of VAR, TYP,
     *            PCK, REGION).
     */
	...
	if ((kind & RPL_ELT) != 0) { // DPJ
	    ...
	    // return new VarRegionSymbol(sym.flags_field, (VarSymbol)sym,
	    // sym.owner);
	    else if (sym.kind < bestSoFar.kind)
		bestSoFar = sym; // DPJ
	}
	...
	if (sym.kind >= AMBIGUOUS) {
	    ...
	    if (sym == syms.errSymbol // preserve the symbol name through errors
		    || ((sym.kind & ERRONEOUS) == 0 // make sure an error symbol
						    // is returned
		    && (sym.kind & TYP) != 0))
    ...
    Symbol access(Symbol sym, DiagnosticPosition pos, Type site, Name name,
	    boolean qualified) {
	if (sym.kind >= AMBIGUOUS)
	    return access(sym, pos, site, name, qualified, List.<Type> nil(),
		    null);
	else
	    return sym;
    }
	...
	if ((sym.flags() & ABSTRACT) != 0)
	    log.error(pos, "abstract.cant.be.accessed.directly", kindName(sym),
		    sym, sym.location());
    }
     ...
     */
    Symbol resolveMethod(DiagnosticPosition pos, Env<AttrContext> env,
	    Name name, List<Type> argtypes, List<Type> typeargtypes,
	    List<RPL> regionargs, List<Effects> effectargs) {
	Symbol sym = findFun(env, name, argtypes, typeargtypes, regionargs,
		effectargs, false, env.info.varArgs = false);
	...
	if (sym.kind == MTH)
	    ...
	    throw new FatalError(JCDiagnostic.fragment(
		    "fatal.err.cant.locate.meth", name));
    }
	...
	if (varargsEnabled && sym.kind >= WRONG_MTHS) {
	    ...
	    if (sym.kind >= WRONG_MTHS)
		sym = resolveConstructor(pos, env, site, argtypes,
			typeargtypes, regionargs, effectargs, true,
			env.info.varArgs = true);
	}
	if (sym.kind >= AMBIGUOUS) {
	    sym = access(sym, pos, site, names.init, true, argtypes,
		    typeargtypes);
	}
	return sym;
    }
	...
	if ((sym.flags() & DEPRECATED) != 0
		...
		&& env.info.scope.owner.outermostClass() != sym
			.outermostClass())
	...
	if (sym.kind == MTH)
	    ...
	    throw new FatalError(JCDiagnostic.fragment(
		    "fatal.err.cant.locate.ctor", site));
    }
	...
	return access(sym, pos, env.enclClass.sym.type, name, false, argtypes,
		null);
    }
     ...
     */
    Symbol resolveBinaryOperator(DiagnosticPosition pos, int optag,
	...
	default:
	    return JCDiagnostic.fragment("kindname", Integer.toString(kind)); // debug
	}
    }
	...
	if (t.tag == TYPEVAR || t.tag == CLASS
		&& (t.tsym.flags() & COMPOUND) != 0)
	...
	AccessError error = new AccessError(env, type.getEnclosingType(),
		type.tsym);
	...
	public String toString() {
	    return debugName + " wrongSym=" + wrongSym + " explanation="
		    + explanation;
	}
	 ...
	 * @param argtypes
	 *            The invocation's value arguments, if we looked for a
	 *            method.
	 * @param typeargtypes
	 *            The invocation's type arguments, if we looked for a
	 *            method.
	 */
	void report(Log log, DiagnosticPosition pos, Type site, Name name,
		...
		if (kind >= WRONG_MTHS && kind <= ABSENT_MTH) {
			...
			kindname = JCDiagnostic
				.fragment("kindname.constructor");
		...
		if (kind == WRONG_MTH) {
		    log.error(pos, "cant.apply.symbol"
			    ...
			    wrongSym.location(site, types), typeargs,
			    Type.toString(argtypes), explanation);
		} else if (site.tsym.name.len != 0) {
		    if (site.tsym.kind == PCK && !site.tsym.exists())
			log.error(pos, "doesnt.exist", site.tsym);
		    else {
			log.error(pos, "cant.resolve.location", kindname,
				idname, args, typeargs, typeKindName(site),
				site);
		    }
		} else {
	...
	AccessError(Env<AttrContext> env, Type site, Symbol sym) {
	    ...
	    this.site = site;
	    if (debugResolve)
		log.error("proc.messager", sym + " @ " + site
			+ " is inaccessible.");
	}
	...
	void report(Log log, DiagnosticPosition pos, Type site, Name name,
		...
		if ((sym.flags() & PUBLIC) != 0
			|| (env != null && this.site != null && !isAccessible(
				env, this.site)))
		    ...
		    log.error(
			    ...
			    TreeInfo.flagNames(sym.flags()
				    & (PRIVATE | PROTECTED)), sym.location());
		else
		    log.error(pos, "not.def.public.cant.access", sym,
			    sym.location());
	    }
	}
    }
	...
	void report(Log log, DiagnosticPosition pos, Type site, Name name,
		List<Type> argtypes, List<Type> typeargtypes) {
	    String symstr = ((sym.kind == TYP && sym.type.tag == CLASS) ? types
		    .erasure(sym.type) : sym).toString();
	    log.error(pos, "non-static.cant.be.ref", kindName(sym), symstr);
	}
    }
	...
	void report(Log log, DiagnosticPosition pos, Type site, Name name,
	    ...
	    log.error(pos, "ref.ambiguous", sname, kindName(pair.sym1),
		    pair.sym1, pair.sym1.location(site, types),
		    kindName(pair.sym2), pair.sym2,
