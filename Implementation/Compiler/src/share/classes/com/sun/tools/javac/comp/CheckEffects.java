...
public class CheckEffects extends EnvScanner { // DPJ
    ...
    protected static final Context.Key<CheckEffects> effectsKey = new Context.Key<CheckEffects>();
    ...
    private boolean statementsInterfere(JCStatement stat,
	    List<JCStatement> stats, boolean atomicOK) {
	for (JCStatement stat2 : stats) {
	    Effects effects1 = stat.effects.inEnvironment(rs, childEnvs.head,
		    false);
	    Effects effects2 = stat2.effects.inEnvironment(rs, childEnvs.head,
		    false);
    ...
    /**
     * Utility class to compute the accessed RPL of an expression that accesses
     * a location. The expression itself doesn't tell us whether the access is a
     * read or write; we need the surrounding context (parent in the AST) to
     * tell us that.
     */
	...
	private boolean isInstanceField(VarSymbol v) {
	    ...
	    if (v.owner.kind == TYP && (v.flags() & STATIC) == 0
	...
	public void visitSelect(JCFieldAccess tree) {
		...
		if (inConstructor && isInstanceField(vsym))
		    return;
		if (vsym.rpl == null)
		    return;
		result = rpls.memberRPL(types, ct, vsym).substForAllParams(ct);
	...
	public void visitIdent(JCIdent tree) {
		...
		if (!inConstructor || !isInstanceField(v)) {
		    result = rpls.memberRPL(types,
			    parentEnv.enclClass.sym.type, (VarSymbol) tree.sym);
		}
	    }
	}
    ...
    private void addAll(List<? extends JCTreeWithEffects> from,
	    JCTreeWithEffects to) {
	for (List<? extends JCTreeWithEffects> l = from; l.tail != null; l = l.tail) {
	    to.effects.addAll(l.head.effects);
	    if (inConstructor(parentEnv))
		to.getConstructorEffects().addAll(
			l.head.getConstructorEffects());
	}
    }

    /**
     * Add the effects of from into the effects of to If the tree accessed an
     * RPL, add a read effect for it
     * 
     * @param from
     * @param to
     */
    ...
    /**
     * If from accessed an RPL, add a read effect for it to the effects of to.
     * 
     * @param from
     * @param to
     */
	...
	if (access != null)
	    to.effects.add(new Effect.ReadEffect(rpls, access, inAtomic,
		    inNonint));
	access = accessedRPL(from, true);
	if (access != null && inConstructor(parentEnv))
	    to.getConstructorEffects().add(
		    new Effect.ReadEffect(rpls, access, inAtomic, inNonint));
    }

    /**
     * Add the effects of from into the effects of to. If the tree accessed an
     * RPL, add a write effect for it.
     * 
     * @param from
     * @param to
     */
    ...
    /**
     * If from accessed an RPL, add a write effect for it to the effects of to.
     * 
     * @param from
     * @param to
     */
	...
	RPL access = accessedRPL(from, false);
	if (access != null)
	    to.effects.add(new Effect.WriteEffect(rpls, access, inAtomic,
		    inNonint));
	access = accessedRPL(from, true);
	if (access != null)
	    to.getConstructorEffects().add(
		    new Effect.WriteEffect(rpls, access, inAtomic, inNonint));
    }
	...
	if (tree.body != null) {
		...
		actualEffects = tree.body.getConstructorEffects()
			.inEnvironment(rs, childEnvs.head, true);
	    }
	}
	...
	if (tree.var.init != null)
	    addAllWithRead(tree.var.init, tree);
	if (tree.start != null)
	    addAllWithRead(tree.start, tree);
	if (tree.length != null)
	    addAllWithRead(tree.length, tree);
	if (tree.stride != null)
	    addAllWithRead(tree.stride, tree);
	addAll(tree.body, tree);
	...
	Effects negatedEffects = effects
		.substIndices(List.of(tree.var.sym), List
			.<JCExpression> of(new DPJNegationExpression(
				tree.var.sym)));
    ...
    private void accumulateAssignEffects(JCExpression lhs, JCExpression rhs,
	    JCTreeWithEffects to) {
	...
	if (sym != null) {
	    Effects effects = sym.effects.translateMethodEffects(tree, types,
		    attr, parentEnv);
